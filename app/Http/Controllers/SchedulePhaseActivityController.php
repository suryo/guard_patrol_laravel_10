...
class SchedulePhaseActivityController extends Controller
{
    ...
    public function list(Request $r, $phase_uid)
{
    // Pastikan phase ada
    $phase = TbPhase::findOrFail($phase_uid);

    // Ambil konteks group dari query
    $scheduleGroupUid = $r->integer('schedule_group_uid');

    if ($scheduleGroupUid) {
        // Cari baris pivot schedule_group_phase
        $sgPhaseUid = DB::table('tb_schedule_group_phase')
            ->where('schedule_group_uid', $scheduleGroupUid)
            ->where('phase_uid', $phase->uid)
            ->value('uid');
        ...
        // Ambil activities untuk kombinasi group-phase ini saja
        $rows = \App\Models\TbScheduleGroupPhaseActivity::with([
                'taskGroup',
                'taskGroupDetail',
                'task',
            ])
            ->where('schedule_group_phase_uid', $sgPhaseUid)
            ->orderByRaw('COALESCE(sortOrder, 999999), uid')
            ->get();
    ...
    // Fallback lama (jika schedule_group_uid tidak dikirim): kembalikan semua activity milik phase
    // Catatan: ini bisa mencampur beberapa group; idealnya front-end SELALU mengirim schedule_group_uid.
    $rows = \App\Models\TbScheduleGroupPhaseActivity::with([
            'taskGroup',
            'taskGroupDetail',
            'task',
