...
class TbUserController extends Controller
{
    ...
    {
        $items = TbUser::orderBy('userName')->paginate(15);
    ...
    public function store(Request $r)
    {
        $v = $r->validate([
            ...
            'hashMobile' => 'nullable|max:255',
            'hashWeb' => 'nullable|max:255',
        ]);
        $v['userPassword'] = Hash::make($v['userPassword']);
        $v['lastUpdated'] = now();
        TbUser::create($v);
        return redirect()->route('users.index')->with('ok', 'Created');
    }
    ...
    public function update(Request $r, TbUser $user)
    {
        ...
        if (!empty($v['userPassword'])) $v['userPassword'] = Hash::make($v['userPassword']);
        else unset($v['userPassword']);
        $user->update($v + ['lastUpdated' => now()]);
        return redirect()->route('users.index')->with('ok', 'Updated');
    }
