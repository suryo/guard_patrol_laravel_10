...
// helper: pakai satu instance Bootstrap
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function addActivityInit() {
    ...
    // Klik tombol + Add Activity (delegated)
    document.addEventListener("click", (e) => {
      ...
      if (!groupUid) {
        ...
        if (host) {
          groupUid = host.dataset?.group || (host.id?.startsWith("group-content-")
            ? host.id.replace("group-content-", "")
            : "");
        }
      }
      ...
      if (sgInput) sgInput.value = String(groupUid);
      ...
      formAdd.addEventListener("submit", async (ev) => {
        ...
        try {
          const res = await fetch(`/phase/${phaseUid}/activities`, {
            ...
            body: JSON.stringify({ schedule_group_uid: parseInt(groupUid, 10), activities }),
          });
          ...
          // Tutup modal & refresh tampilan (sementara full reload; bisa diubah ke partial refresh)
          modal.hide();
