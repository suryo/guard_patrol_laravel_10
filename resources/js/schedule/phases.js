    ...
    .map((a) => {
      ...
      return `
        <div class="activity-box mb-2">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              <div class="kv"><div class="k">PIC</div><div class="v">: ${esc(pic)}</div></div>
              <div class="kv"><div class="k">Lokasi</div><div class="v">: ${esc(where)}</div></div>
              <div class="kv"><div class="k">Waktu</div><div class="v">: ${esc(time)}</div></div>
            </div>
            <div class="col-md-6">
              <div class="kv"><div class="k">Catatan</div><div class="v">: ${note ? esc(note) : '-'}</div></div>
            </div>
          </div>
          <div class="mt-2 d-flex justify-content-end">
            <button class="btn btn-sm btn-outline-danger btn-del-activity" data-act="${esc(a.uid ?? a.id)}">
              Hapus
            </button>
          </div>
        </div>
...
function phaseListHTML(phases, groupUid, date) {
  if (!phases || phases.length === 0) {
    return `
      ...
      <button class="btn btn-sm btn-primary add-phase" data-group="${groupUid}" data-date="${esc(
        date
      )}">
        Tambah Phase
      </button>
  ...
  // satu accordion untuk daftar phase
  const items = phases
    .map((p) => {
              ...
              <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <div class="btn-group btn-group-sm">
                  ...
                  <button class="btn btn-outline-secondary edit-phase"
                    data-link="${linkUid}" data-group="${groupUid}" data-date="${esc(
        p.phaseDate
      )}" data-phase="${pid}">
                    Edit
                  </button>
              ...
              <!-- INNER: Activities Accordion (satu panel 'Activities' yang lazy loaded) -->
                ...
                <div class="accordion-item border">
                  <h2 class="accordion-header">
                    <button
                      ...
                      data-bs-toggle="collapse"
                      data-bs-target="#act-panel-${groupUid}-${pid}-${linkUid}"
                      data-phase="${pid}">
                      Activities
                    </button>
                  </h2>
                  <div id="act-panel-${groupUid}-${pid}-${linkUid}"
  ...
  return `
    ...
    <button class="btn btn-sm btn-primary mt-2 add-phase" data-group="${groupUid}" data-date="${esc(
    date
  )}">
      Tambah Phase
    </button>
...
async function loadPhaseActivities(phaseUid, panelEl) {
  ...
  try {
    ...
    const countBadge = accItem?.querySelector(".act-count[data-phase='" + phaseUid + "']");
